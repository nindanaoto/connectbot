...
static int create_mosh_subprocess(
    ...
    const char* terminfo, const char* locale, long* pProcessId) {
  char devname[32];
  int ptm;
  pid_t pid;
  ...
  if (pid == 0) {
    ...
    execlp("mosh-client", "mosh-client", ip, port, NULL);

    // If execlp fails, log and exit
    LOG("[ execlp mosh-client failed: %s ]\n", strerror(errno));
    exit(-1);
  } else {
    *pProcessId = (long) pid;
    return ptm;
...
JNIEXPORT jobject JNICALL Java_org_mosh_MoshClient_forkExec(
    JNIEnv* env, jclass clazz, jstring ip, jstring port, jstring key,
    jstring terminfo, jstring locale, jlongArray processIdArray) {
  ...
  long procId;
  int ptm = create_mosh_subprocess(ip_8, port_8, key_8, terminfo_8, locale_8, &procId);
