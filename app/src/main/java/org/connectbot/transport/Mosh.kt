 ...
 * 5. Fork native mosh-client process to connect via UDP
 *
...
class Mosh : SSH {
    ...
    override fun connect() {
        ...
        if (!InstallMosh.waitForInstall(10000)) {
        ...
        try {
            ...
            // Close SSH session (mosh-server is now running independently)
            closeSSHSession()
    ...
    private fun launchMoshServer(currentHost: Host): MoshCredentials? {
        try {
            ...
            val matcher = MOSH_CONNECT_PATTERN.matcher(output)
            if (!matcher.find()) {
                Timber.e("Failed to parse MOSH CONNECT from output: $output")
                return null
            }

            val port = matcher.group(1) ?: return null
            val key = matcher.group(2) ?: return null
    ...
    private fun buildMoshServerCommand(currentHost: Host): String {
        ...
        // Set locale and run mosh-server
        // The 'new' argument tells mosh-server this is a new session
        return "LANG=$locale LC_ALL=$locale $serverCmd$portArg new"
    }

    /**
     * Close the SSH session but keep the connection info for reference.
     */
    private fun closeSSHSession() {
        session?.close()
        session = null
        sessionOpen = false
        stdin = null
    ...
    /**
     * Start the native mosh-client process.
     *
    ...
    private fun startMoshClient(credentials: MoshCredentials, currentHost: Host): Boolean {
        try {
            val terminfoPath = InstallMosh.getTerminfoPath()
                ?: manager?.filesDir?.absolutePath ?: "/data/local/tmp"
